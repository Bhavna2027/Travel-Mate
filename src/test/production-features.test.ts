...
async function runTests() {
  ...
  try {
    ...
    const existingGuide = await prisma.users.findUnique({ where: { phone: '9777777777' } });
    if (existingGuide) {
      await prisma.bookings.deleteMany({ where: { guide_id: existingGuide.user_id } });
      await prisma.guide_profiles.deleteMany({ where: { guide_id: existingGuide.user_id } });
    }

    // Find test user ID if exists
    const existingUser = await prisma.users.findFirst({ where: { phone: testUser.phone } });
    if (existingUser) {
      await prisma.group_members.deleteMany({ where: { user_id: existingUser.user_id } });
    }

    await prisma.groups.deleteMany({ where: { destination: 'Manali' } });
    await prisma.users.deleteMany({ where: { phone: { in: [testUser.phone, '9777777777'] } } });
    ...
    const userDb = await prisma.users.findUnique({ where: { phone: testUser.phone } });
    const dbPhoneOtp = (userDb?.device_info as any)?.otp?.code;
    ...
    const aadhaarOtpData = (await aadhaarOtpRes.json()) as any;
    ...
    const userWithAadhaar = await prisma.users.findUnique({ where: { user_id: regData.user_id } });
    const aadhaarOtp = (userWithAadhaar?.device_info as any)?.aadhaar_verification?.otp_code;
    if (!aadhaarOtp) {
    ...
    const dest = await prisma.destinations.upsert({
      where: { name: 'Manali' },
      update: {},
    ...
    const guideUser = await prisma.users.create({
      data: {
        phone: '9777777777',
    ...
    await prisma.guide_profiles.create({
      data: {
    ...
    const orderData = (await bookOrderRes.json()) as any;
    ...
    const remindersData = (await remindersRes.json()) as any;
