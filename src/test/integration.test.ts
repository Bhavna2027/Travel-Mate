...
async function runTests() {
  ...
  try {
    ...
    for (const tu of testUsers) {
      ...
      const userDb = await prisma.users.findUnique({ where: { phone: tu.phone } });
    ...
    const itinData = (await itinRes.json()) as any;
    ...
    for (const token of reporters) {
      ...
      lastReportData = (await repRes.json()) as any;
    }
    ...
    for (const uid of userIds) {
      const user = await prisma.users.findUnique({ where: { user_id: uid } });
      if (user) {
