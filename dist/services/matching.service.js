...
exports.calculatePairwiseScore = calculatePairwiseScore;
exports.runMatchingProcess = runMatchingProcess;
...
function calculatePairwiseScore(uA, uB) {
    ...
    const budgetTiers = { low: 0, mid: 1, high: 2 };
...
function checkGenderConstraint(candidate, group) {
    ...
    for (const m of tempGroup) {
        if (m.gender_preference === 'women-only' && genders.some(g => g !== 'F'))
            return false;
        if (m.gender_preference === 'men-only' && genders.some(g => g !== 'M'))
            return false;
    }
    return true;
}
// Compute average compatibility score for a group
function getGroupCompatibility(groupIds, scoreMap) {
...
async function runMatchingProcess() {
    ...
    return await client_1.prisma.$transaction(async (tx) => {
        const lockResult = await tx.$queryRaw `SELECT pg_try_advisory_xact_lock(1337) as locked`;
        ...
        for (const [dest, pool] of destGroups.entries()) {
            ...
            const candidatesMap = new Map(pool.map(c => [c.userId, c]));
            const formedGroups = [];
            // Greedy packing
                    ...
                    for (let j = i + 1; j < formedGroups.length; j++) {
                        ...
                        for (let idxI = 0; idxI < groupI.length; idxI++) {
                            for (let idxJ = 0; idxJ < groupJ.length; idxJ++) {
                                ...
                                if (!checkGenderConstraint(candidatesMap.get(uJ), groupI.filter(id => id !== uI).map(id => candidatesMap.get(id))))
                                    continue;
                                if (!checkGenderConstraint(candidatesMap.get(uI), groupJ.filter(id => id !== uJ).map(id => candidatesMap.get(id))))
                                    continue;
            ...
            for (const group of formedGroups) {
                ...
                const buddiesMap = new Map(); // user_id -> buddy role/id
