...
async function runNegativeTests() {
    ...
    const userDb = await client_1.prisma.users.findUnique({ where: { phone: '9900000010' } });
    ...
    if (missingFramesRes.status !== 400)
        throw new Error('Expected 400 on missing frames');
    ...
    if (invalidFramesRes.status !== 400)
        throw new Error('Expected 400 on invalid frames');
