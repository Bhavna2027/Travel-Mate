...
async function getAdminDashboard(req, res) {
    try {
        ...
        const pendingReports = await client_1.prisma.reports.findMany({
        ...
        // Soft-deleted users
        const softDeletedUsers = await client_1.prisma.users.findMany({
            where: { deleted_at: { not: null } },
            select: { user_id: true, name: true, phone: true, email: true, deleted_at: true }
        });
        // Group health/lifecycle status
        const groupStats = await client_1.prisma.groups.findMany({
            orderBy: { created_at: 'desc' }
        });
        // Recent critical audit logs
        const recentAudits = await client_1.prisma.audit_logs.findMany({
            take: 20,
            orderBy: { created_at: 'desc' },
            include: {
...
async function listFeatureFlags(req, res) {
    try {
        const flags = await client_1.prisma.feature_flags.findMany();
...
async function upsertFeatureFlag(req, res) {
    try {
        ...
        const flag = await client_1.prisma.feature_flags.upsert({
            where: { name },
            update: {
