...
exports.bookGuide = bookGuide;
exports.verifyGuideBooking = verifyGuideBooking;
...
async function listGuides(req, res) {
    try {
        const guides = await client_1.prisma.guide_profiles.findMany({
            where: {
...
async function requestGuide(req, res) {
    try {
        ...
        const guide = await client_1.prisma.guide_profiles.findUnique({
            where: { guide_id }
        });
        if (!guide) {
        ...
        // Create request
        const request = await client_1.prisma.guide_requests.create({
            data: {
                group_id,
                guide_id,
                requested_by: userId,
...
async function bookGuide(req, res) {
    try {
        const userId = req.user?.user_id;
        if (!userId) {
            res.status(401).json({ code: 'UNAUTHORIZED', message: 'Auth required.' });
        ...
        const order = await razorpay_service_1.razorpayService.createOrder(parsedAmount, receipt);
        res.status(201).json({
...
async function verifyGuideBooking(req, res) {
    try {
        const userId = req.user?.user_id;
        if (!userId) {
            res.status(401).json({ code: 'UNAUTHORIZED', message: 'Auth required.' });
        ...
        const { group_id, guide_id, guide_request_id, amount, razorpay_order_id, razorpay_payment_id, razorpay_signature } = req.body;
        ...
        // Verify signature
        const isValidSignature = razorpay_service_1.razorpayService.verifyPaymentSignature(razorpay_order_id, razorpay_payment_id, razorpay_signature || '');
        if (!isValidSignature) {
