...
exports.checkFace = void 0;
exports.getKycChallenge = getKycChallenge;
exports.verifyLivenessOnly = verifyLivenessOnly;
exports.verifyKyc = verifyKyc;
exports.requestAadhaarOtp = requestAadhaarOtp;
exports.confirmAadhaarOtp = confirmAadhaarOtp;
exports.triggerSos = triggerSos;
exports.shareLocation = shareLocation;
exports.getGroupLocations = getGroupLocations;
...
async function getKycChallenge(req, res) {
    try {
...
const checkFace = async (req, res) => {
    try {
        ...
        const faceDetected = await (0, adhar_service_1.checkFacePresence)(frame);
...
async function verifyLivenessOnly(req, res) {
    try {
...
async function requestAadhaarOtp(req, res) {
    try {
        ...
        await notification_service_1.notificationService.sendAadhaarOtp(user.phone, user.email, otpCode);
        res.status(200).json({
...
async function triggerSos(req, res) {
    try {
        ...
        const alert = await client_1.prisma.sos_alerts.create({
            data: {
                user_id: userId,
        ...
        const contacts = await client_1.prisma.emergency_contacts.findMany({
            where: { user_id: userId }
        });
        ...
        for (const contact of contacts) {
            await (0, sms_service_1.sendSMS)(contact.phone, alertBody);
        }
...
async function shareLocation(req, res) {
    try {
        ...
        const existingConsent = await client_1.prisma.consent_records.findFirst({
            where: { user_id: userId, consent_type: 'location', granted: true }
        });
        if (!existingConsent) {
            await client_1.prisma.consent_records.create({
                data: {
                    user_id: userId,
        ...
        const existingShare = await client_1.prisma.location_shares.findFirst({
            where: { user_id: userId, group_id: group_id || null }
        });
        let share;
        if (existingShare) {
            share = await client_1.prisma.location_shares.update({
                where: { share_id: existingShare.share_id },
                data: { latitude, longitude, accuracy_meters, shared_at: new Date(), expires_at: expiresAt }
            });
        }
        else {
            share = await client_1.prisma.location_shares.create({
                data: {
                    user_id: userId,
                    group_id: group_id || null,
                    latitude,
...
async function getGroupLocations(req, res) {
    try {
        ...
        const isMember = await client_1.prisma.group_members.findFirst({
        ...
        const activeShares = await client_1.prisma.location_shares.findMany({
            where: {
                group_id,
...
async function submitReport(req, res) {
    try {
        ...
        // Insert Report
        const report = await client_1.prisma.reports.create({
            data: {
                reporter_id: userId,
                reported_user_id,
                group_id: group_id || null,
        ...
        const reportCount = await client_1.prisma.reports.count({
        ...
        if (reportCount >= 3) {
            ...
            await client_1.prisma.$transaction([
                client_1.prisma.users.update({
                    where: { user_id: reported_user_id },
                    data: { deleted_at: new Date() }
                }),
                client_1.prisma.sessions.deleteMany({
                    where: { user_id: reported_user_id }
                })
            ]);
            autoActioned = true;
