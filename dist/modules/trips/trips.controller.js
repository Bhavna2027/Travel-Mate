...
async function createTrip(req, res) {
    try {
        ...
        const { destination, start_date, end_date, budget_tier, interests, preferred_group_size_min, preferred_group_size_max } = req.body;
        // Validate inputs
        ...
        const newTrip = await client_1.prisma.trips.create({
            data: {
                user_id: userId,
                destination,
                start_date: start,
...
async function listTrips(req, res) {
    try {
        ...
        const userTrips = await client_1.prisma.trips.findMany({
            where: {
                user_id: userId,
                status: 'open'
            },
...
async function updateTrip(req, res) {
    try {
        ...
        const trip = await client_1.prisma.trips.findUnique({ where: { trip_id } });
        ...
        const updatedTrip = await client_1.prisma.trips.update({
            where: { trip_id },
            data: updateData
        });
        res.status(200).json({
...
async function closeTrip(req, res) {
    try {
        ...
        const closedTrip = await client_1.prisma.trips.update({
            where: { trip_id },
            data: { status: 'closed' }
        });
        res.status(200).json({
...
async function sendTripReminders(req, res) {
    try {
        const userId = req.user?.user_id;
        if (!userId) {
            res.status(401).json({ code: 'UNAUTHORIZED', message: 'Not authenticated.' });
        ...
        const groups = await client_1.prisma.groups.findMany({
            where: {
        ...
        for (const g of groups) {
            ...
            for (const m of g.group_members) {
                ...
                await notification_service_1.notificationService.sendTripReminder(m.users.email, m.users.phone, m.users.name, g.destination, formattedDate, otherBuddies);
                remindersSent++;
            }
        }
        res.status(200).json({
